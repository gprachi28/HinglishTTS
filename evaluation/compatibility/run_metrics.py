...
"""
Master evaluation metrics runner — HinglishTTS-Bench Phase 1.5.

Canonical entry point for computing all 5 linguistic quality metrics
for a given model. Runs metrics in dependency order:

  1. TPI   — ASR-proxy Transliteration Penalty Index (also generates transcripts.json)
  2. PIER  — Point-of-Interest Error Rate at code-switch boundaries
  3. H-Index — Hindi Phonetic Fidelity (fraction of HI tokens correctly transcribed)
  4. F0    — Boundary F0 RMSE proxy (F0 std-dev across voiced frames)
  5. LID   — Language Boundary Confidence (script-type accuracy at switch points)

All results saved to:
    evaluation/compatibility/results/{model}/
      transcripts.json    — Whisper ASR transcripts (generated by step 1)
      tpi.json            — WER per variant + TPI
      pier_mixed.json     — PIER at code-switch boundaries
      hindex.json         — H-Index per sentence and category
      f0.json             — F0 std-dev per file and variant
      lid.json            — LID confidence per sentence
      metrics_summary.json — Combined summary of all metrics

Usage:
    python -m evaluation.compatibility.run_metrics --model cosyvoice3
    python -m evaluation.compatibility.run_metrics --model cosyvoice3 --whisper-model large-v3
    python -m evaluation.compatibility.run_metrics --model cosyvoice3 --skip-transcription
"""
