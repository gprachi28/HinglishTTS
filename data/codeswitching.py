...
from data.vocabulary import (  # noqa: F401 — re-exported for backward compat
    ...
    EN_VERBS_QUICK_TASK, EN_VERBS_SOCIAL, EN_VERBS_WORK, HI_ADJECTIVES_PURE,
    HI_CONNECTORS, HI_DISCOURSE, HI_EMOTIONS, HI_INTENSIFIERS, HI_SUBJECTS,
    HI_SUBJECTS_FIRST_PERSON, HI_SUBJECTS_SOCIAL, HI_TIME, HI_TIME_LONG,
    HI_TIME_SHORT, HI_VERBS_FUTURE, HI_VERBS_GENERAL, Lang, TaggedToken, en,
    hi, r)
...
def build_hindi_matrix_english_verb() -> List[TaggedToken]:
    patterns = [
        ...
        lambda: hi("main")
        + hi("pehle")
        + r(EN_VERBS_QUICK_TASK)
        + hi("karke aata hoon"),
        lambda: hi("Kya tum") + r(EN_VERBS_ALL) + hi("kar sakte ho"),
        lambda: hi("Yeh")
        + r(EN_VERBS_EVERYDAY + EN_VERBS_SOCIAL)
        + hi("karna itna mushkil nahi hai"),
...
def build_english_matrix_hindi_np() -> List[TaggedToken]:
    patterns = [
        ...
        + r(EN_VERBS_WORK)
        + r(HI_TIME_LONG),
        lambda: en("I was thinking we should")
        + r(EN_VERBS_ALL)
        + en("the")
        + r(EN_NP_EVERYDAY)
        + r(HI_TIME),
        lambda: en("Could you please")
        + r(EN_VERBS_ALL)
        + en("the")
        + r(EN_NP_WORK)
        + r(HI_TIME_SHORT)
...
def build_inter_sentential() -> List[TaggedToken]:
    patterns = [
        lambda: r(HI_SUBJECTS_FIRST_PERSON)
        + hi("soch raha tha.")
        + en("The")
        + r(EN_NP_ALL)
        + en("is really")
        ...
        lambda: r(HI_EMOTIONS)
        + en(". The")
        + r(EN_NP_EVERYDAY)
        + en("was really")
        + r(EN_ADJECTIVES),
        lambda: hi("Bahut thak gaya hoon.")
        + en("This")
        + r(EN_NP_EVERYDAY)
        + en("is not helping"),
        lambda: hi("Samajh nahi aa raha.")
        + en("The")
        + r(EN_NP_TECH)
        + en("keeps giving errors"),
...
def build_everyday_conversation() -> List[TaggedToken]:
    patterns = [
        ...
        lambda: r(HI_SUBJECTS_SOCIAL)
        + en("lunch")
        + hi("ke liye kahan chalein")
        + r(HI_TIME_SHORT),
        lambda: hi("Aaj")
        + en("coffee")
        + hi("peeni hai,")
        + r(HI_SUBJECTS_SOCIAL)
        + hi("aa raha hai kya"),
        lambda: hi("Kal")
        + en("party")
        + hi("mein")
        ...
        + hi("hai,")
        + r(HI_SUBJECTS_SOCIAL)
        + hi("packing nahi ki abhi tak"),
        # Health
        lambda: r(HI_SUBJECTS_SOCIAL)
        + en("gym")
        + hi("jaana")
        + r(HI_INTENSIFIERS)
        + hi("mushkil ho gaya hai"),
        ...
        + r(HI_INTENSIFIERS)
        + en("slow")
        + hi("hai, kaam nahi ho raha"),
        # FIX: en("zoom call"), en("connection") not inside hi()
        lambda: r(HI_SUBJECTS_FIRST_PERSON)
        + en("zoom call")
        + hi("pe tha,")
        + en("connection")
        + hi("baar baar jaata tha"),
    ]
    return r(patterns)()
